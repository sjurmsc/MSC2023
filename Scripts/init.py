...
if __name__ == '__main__':
    ...
    while Path(distance_loss).exists():
        distance_loss = distance_loss[:22] + str(int(distance_loss[22:-4]) + 1) + '.csv'

    with open(distance_loss, 'w') as f:
        f.write(f'max_distance, mae, gname\n')

    for maxdistance in m_dist:
        ...
        dataset_params = {
            ...
            'n_bootstraps'          : 5,
            'add_noise'             : False,
            'max_distance_to_cdp'   : maxdistance,
        ...
        for i, (train_index, test_index) in enumerate(cv.split(X_train, y_train, groups_train)):
            ...
            if len(GGM_test_not_in_train) > 0:
                ...
                with open(f'./Models/{gname}/Fold{i+1}/GGM_test_not_in_train.txt', 'w') as f:
                    f.write(umap(GGM_test_not_in_train))
            ...
            metrics = model.evaluate(X_test_cv, [y_test_cv, X_test_cv], verbose=0)
            for j, metric in enumerate(model.metrics_names):
                loss_dict[f'Fold{i+1}_{metric}'] = metrics[j]

            training_time_dict[i]['CNN'] = time() - t0

            Histories.append(History)

            with open(distance_loss, 'a') as f:
                f.write(f'{maxdistance}, {metrics[1]}, {gname}\n')
