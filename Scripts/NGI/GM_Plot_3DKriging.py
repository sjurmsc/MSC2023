...
def plot_data_fit_UK3D(ax, X, y_pred):
    ax.plot(y_pred, X, 'g', linestyle='-', linewidth=0.75)
    return ax


def plot_data_pred(df, df_up, df_RF_scores, df_RF_reg, df_unit_col, path_pdf):
    ...
    # Loop over locations
    for loc in loclist:
    # for loc in [9, 19, 25, 41, 61, 76, 77, 78, 81]:
    # for loc in [7, 15, 24, 41, 42, 49, 50, 55, 76, 78]:
        if loc <= 84:
            loc_txt = 'TNW%03d' %loc 
        else:
        ...
        print('Loc:', int(loc))
        # path png
        path_png = '../../09-Results/Stage-01/Final/Plots_UK3D/UK3D-Location_%s.png' %(loc_txt)
        path_png = '../../09-Results/Stage-03/qc_pred_UK3D/Plots/UK3D-Location_%s.png' %(loc_txt)
        # get all CPT at this location
        df_loc = df.loc[df['ID']==loc,:]
        df_loc_U = df_up.loc[df_up['ID']==loc,:]
        ...
        fs_U = df_loc_U['fs']
        u2_U = df_loc_U['u2']
        axs[0].plot(qc_U, z_U, 'm:')
        axs[1].plot(fs_U, z_U, 'm:')
        axs[2].plot(u2_U, z_U, 'm:')   
        ...
        for ii, feature in zip([0],['qc']):
            ...
            # X_pred = df_RF_reg.loc[(df_RF_reg['feature']==feature) & (df_RF_reg['ID']==loc), 'z_bsf'].values
            # y_true = df_RF_reg.loc[(df_RF_reg['feature']==feature) & (df_RF_reg['ID']==loc), 'y_true'].values
            # y_pred = df_RF_reg.loc[(df_RF_reg['feature']==feature) & (df_RF_reg['ID']==loc), 'y_pred_loo'].values
            axs[ii] = plot_data_fit_RF(axs[ii], X_pred, y_true, y_pred) 
            # PLot fullscale UK3D prediction
            unitlist = df_loc['unit'].dropna().unique()
            for unit in unitlist[:-1]:
                z_loc = df_loc_U.loc[df_loc_U['unit']==unit,['z_bsf']].drop_duplicates().values.reshape(-1,1)
                [UK3D] = df_RF_scores.loc[(df_RF_scores['unit']==unit) & (df_RF_scores['feature']==feature), 'RFreg_obj']
                x_loc = np.repeat(df_loc_U.loc[df_loc_U['unit']==unit,['x']].drop_duplicates().mean().values, len(z_loc)).reshape(-1,1)
                y_loc = np.repeat(df_loc_U.loc[df_loc_U['unit']==unit,['y']].drop_duplicates().mean().values, len(z_loc)).reshape(-1,1)
                UK_pred, _ = UK3D.execute('points', x_loc, y_loc, z_loc)
                axs[ii] = plot_data_fit_UK3D(axs[ii], z_loc, UK_pred)
        ...
        legend_elements = [Line2D([0], [0], color='w', marker='o', markerfacecolor='k', markersize=5, label='UK3D LOO pred'),
                           Line2D([0], [0], color='m', ls=':', lw=.75, label='Upscaled'),
                           Line2D([0], [0], color='g', ls='-', lw=.75, label='UK3D full pred'),]
...
#%% Load upscaled database
path_upscaled = '../../09-Results/Stage-01/Database_upscaled.csv'
df_up = pd.read_csv(path_upscaled)
df_up = df_up[(df_up['z_bsf']>=z_min) & (df_up['z_bsf']<=z_max)]
...
# Path to store PDF
path_pdf = '../../09-Results/Stage-01/3DKriging-Results.pdf'
axs, fig = plot_data_pred(df, df_up, df_UK3D_scores, df_UK3D_reg, df_unit_col, path_pdf)
