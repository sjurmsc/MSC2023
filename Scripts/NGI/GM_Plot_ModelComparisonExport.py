...
def plot_linearfit_pred(axs, df_loc, df_LL_res, df_LL_scores, df_to_store):
        ...
        for unit in df_loc['unit'].dropna().unique():
            ...
            if not df_data.empty:
                ...
                if slope.size>0:
                    ...
                    # df_tmp = df_loc.loc[df_loc['unit']==unit,['z_bsf']].drop_duplicates()
                    # colname = "LL_" + feature
                    # df_tmp[colname] = y_pred             
                    # df_tmp.sort_values(by=['z_bsf'], inplace=True)
                    # df_tmp1 = df_tmp1.append(df_tmp)
                else:
                ...
                df_tmp.sort_values(by=['z_bsf'], inplace=True)
                df_tmp1 = df_tmp1.append(df_tmp)
        df_to_store = pd.merge_asof(df_to_store, df_tmp1, on=['z_bsf'])
    return axs, df_to_store

def plot_globalfit_pred(axs, df_loc, df_GL_res, df_to_store):
    for ii, feature in zip([0, 1, 2],['qc', 'fs', 'u2']):
        # Loop over units
        ...
        for unit in df_loc['unit'].dropna().unique():
            ...
            axs[ii] = plot_data_GL_fit(axs[ii], X_pred, y_pred)                    
                ...
                #                                     df_loc.loc[df_loc['unit']==unit,['y']].drop_duplicates().mean().values)
                    # y_pred = slope*X_pred + intercept*np.ones(np.shape(X_pred))
                    # axs[ii] = plot_data_GL_fit(axs[ii], X_pred, y_pred)
            df_tmp1 = df_loc.loc[df_loc['unit']==unit,['z_bsf']].drop_duplicates()
            colname = "GL_" + feature
            df_tmp1[colname] = y_pred
            df_tmp2 = df_tmp2.append(df_tmp1, ignore_index=True)
            df_tmp2.sort_values(by=['z_bsf'], inplace=True)
        df_to_store = pd.merge_asof(df_to_store, df_tmp2, on=['z_bsf'])
    return axs, df_to_store
...
for loc in loclist:
    ...
    print('Loc:', int(loc))
    #%% Path to store PDF
    # path_pdf = '../../09-Results/Stage-01/Stage-01_ModelComparison_03/ModelComparison-Results-Location_%03d.pdf' %(loc)
    path_png = '../../09-Results/Stage-01/Stage-01_ModelComparison_03/ModelComparison-Results-Location_%s.png' %(loc_txt)
    ...
    fig.suptitle('Model Comparison - Location ' + loc_txt, fontsize=14)
    ...
    axs, df_to_store = plot_linearfit_pred(axs, df_loc, df_LL_res, df_LL_scores, df_to_store)
    ...
    axs, df_to_store = plot_globalfit_pred(axs, df_loc, df_GL_res, df_to_store)
    ...
    # Save df_to_store
    path_tostore = '../../09-Results/Stage-01/Stage-01_ModelComparison_03/ModelComparison-Results-Location_%03d.xlsx' %(loc)
    df_to_store.to_excel(path_tostore)
