...
path_database = '../../09-Results/Stage-01/Database_upscaled.pkl'
df_U = pd.read_pickle(path_database)
...
# pdf = PdfPages('../../09-Results/Stage-01/Database-Upscaled.pdf')

# Loop over locations
for loc in loclist:
    print('Loc: ', int(loc))
    # path png
    path_png = '../../09-Results/Stage-01/Final/Plots_Database/Database-Upscale-Location_%03d.png' %(loc)
    # get all CPT at this location
    df_loc = df.loc[df['ID']==loc,:]
    df_loc_m = df_m.loc[df_m['ID']==loc,:]
    df_loc_U = df_U.loc[df_U['ID']==loc,:]
    ...
    fs_U = df_loc_U['fs']
    u2_U = df_loc_U['u2']
    axs[0].plot(qc_U, z_U, 'm:', label='upscaled')
