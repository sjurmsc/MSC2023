...
def plot_regression_results(fig, ax, y_true, y_pred, feature, unit, scores=[]):
    model_dist, mae, accuracy, mu, std = GMT.evaluate_modeldist_norm(y_true, y_pred)
    if not scores:
...
def plot_global_score(df_RF_reg, df_RF_scores):
    ...
    fig.suptitle('Random Forest Regression - Global scores', fontsize=14)
    unit = 'global'
    for ii, feature in zip([0,1,2], ['qc', 'fs', 'u2']):
        ...
        axs[ii,0] = plot_regression_results(fig, axs[ii,0], y_true, y_pred, feature, unit, scores=scores)
...
def plot_unit_score(df_RF_reg, pdf):
    ...
    for unit in unitlist:
        ...
        fig, axs = plt.subplots(3, 2, figsize=(14,14))
        fig.suptitle('Random Forest Regression - Unit ' + unit + ' scores', fontsize=14)
        for ii, feature in zip([0,1,2], ['qc', 'fs', 'u2']):
...
#%% Main
path_pdf = '../../09-Results/Stage-01/RandomForest-Kfold-Scores.pdf'
pdf = PdfPages(path_pdf)
fig, axs = plot_global_score(df_RF_reg, df_RF_scores)
pdf.savefig(fig)
pdf = plot_unit_score(df_RF_reg, pdf)
pdf.close()  
